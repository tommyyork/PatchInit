...
// and a spectrogram SVG. Finally output one stacked SVG with all plots and labels.
//
...
// ---- STFT: reuse engine FFT and same window ----
    ...
    std::vector<std::vector<float>>& mag_frames)
{
    mag_frames.clear();
    size_t frame = 0;
    for (size_t start = 0; start + kFftSize <= num_samples; start += kHopSize, ++frame)
    {
        std::vector<float> row(kNumBins + 1);
...
{
    return static_cast<float>(bin) * static_cast<float>(kSampleRate) / static_cast<float>(kFftSize);
}
    ...
    size_t num_frames)
{
    ...
    float time_per_frame = static_cast<float>(kHopSize) / static_cast<float>(kSampleRate);
...
int main(int argc, char** argv)
{
    ...
    for (size_t c = 0; c < num_configs; ++c)
    {
        ...
        avg_spectra[c].assign(kNumBins + 1, 0.0f);
        if(nf > 0)
        {
    ...
    // For each configuration, dump both the raw STFT CSV (for debugging) and
    // a note-bucketed, magnitude-sorted CSV for the 100 ms analysis slice.
    for (size_t c = 0; c < num_configs; ++c)
    {
        const Config& cfg = configs[c];

        char csv_path[512], svg_path[512], note_csv_path[512];
        ...
        printf("  %s -> %s, %s, %s\n", cfg.label, csv_path, note_csv_path, svg_path);
    }
